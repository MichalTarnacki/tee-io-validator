...
TEST(FaultInjectionParseTest, ParsesActionsDirectionsAndPatterns)
{
    teeio_fault_action_t action;
    ...
    EXPECT_TRUE(teeio_fault_parse_direction("send", &direction));
    EXPECT_EQ(direction, TEEIO_FAULT_DIRECTION_SEND);
    EXPECT_FALSE(teeio_fault_parse_direction("receive", &direction));
    EXPECT_FALSE(teeio_fault_parse_direction("both", &direction));
...
TEST(FaultInjectionSelectorTest, MatchesSelectorsAndOccurrence)
{
    ...
    teeio_fault_result_t result = teeio_fault_apply(
        TEEIO_FAULT_DIRECTION_SEND, message, sizeof(message), sizeof(message));
...
TEST(FaultInjectionSequenceTest, ReplaysEarlierMessage)
{
    ...
    teeio_fault_init(&config);
    teeio_fault_result_t result = teeio_fault_apply(
        TEEIO_FAULT_DIRECTION_SEND, first, sizeof(first), sizeof(first));
    EXPECT_EQ(result.disposition, TEEIO_FAULT_DISPOSITION_PASS);
    result = teeio_fault_apply(TEEIO_FAULT_DIRECTION_SEND, unrelated,
