...
static teeio_fault_config_t *m_config;
static uint8_t m_primary[TEEIO_FAULT_MAX_MESSAGE_SIZE];
static uint8_t m_secondary[TEEIO_FAULT_MAX_MESSAGE_SIZE];
static uint8_t m_previous[TEEIO_FAULT_DIRECTION_MAX][TEEIO_FAULT_MAX_MESSAGE_SIZE];
static size_t m_previous_size[TEEIO_FAULT_DIRECTION_MAX];
static uint8_t m_held[TEEIO_FAULT_DIRECTION_MAX][TEEIO_FAULT_MAX_MESSAGE_SIZE];
static size_t m_held_size[TEEIO_FAULT_DIRECTION_MAX];
static uint32_t m_total_fire_count;
static char m_audit[512];
static char m_result[512];
...
{
    switch (direction) {
    case TEEIO_FAULT_DIRECTION_SEND:
        return "send";
    case TEEIO_FAULT_DIRECTION_RECEIVE:
        return "receive";
    default:
        return "unknown";
    }
                                 ...
                                 teeio_fault_direction_t *direction)
{
    ...
    if (strcmp(name, "send") == 0) {
        *direction = TEEIO_FAULT_DIRECTION_SEND;
        return true;
    }
    if (strcmp(name, "receive") == 0) {
        *direction = TEEIO_FAULT_DIRECTION_RECEIVE;
        return true;
    }
    return false;
}
                               ...
                               size_t error_size)
{
    ...
    if (rule->direction >= TEEIO_FAULT_DIRECTION_MAX) {
        set_error(error, error_size, "rule %d has invalid direction", rule->id);
        return false;
    }
...
void teeio_fault_reset(void)
{
    ...
    memset(m_held_size, 0, sizeof(m_held_size));
    ...
    for (index = 0; index < m_config->rule_count; index++) {
        ...
        m_config->rules[index].fire_count = 0;
    }
}
                         ...
                         const uint8_t *message, size_t message_size)
{
    ...
    if (m_config->selected_scenario[0] != '\0' &&
        strcmp(rule->scenario, m_config->selected_scenario) != 0) {
        return false;
    }
    if (message_size < TEEIO_DOE_HEADER_SIZE) {
        return false;
                                       ...
                                       size_t message_capacity)
{
    ...
    if (message == NULL || direction >= TEEIO_FAULT_DIRECTION_MAX ||
        message_size > TEEIO_FAULT_MAX_MESSAGE_SIZE ||
    ...
    input = message;
    if (m_config == NULL || !m_config->enabled || m_config->rule_count == 0) {
    ...
    for (index = 0; index < m_config->rule_count; index++) {
        ...
        if (!rule_matches(rule, direction, input, message_size)) {
            continue;
        }
        ...
        if (disposition == TEEIO_FAULT_DISPOSITION_REPLAY) {
            if (m_previous_size[direction] == 0 ||
                m_previous_size[direction] > message_capacity) {
            ...
            if (m_held_size[direction] == 0) {
                memcpy(m_held[direction], input, message_size);
                m_held_size[direction] = message_size;
                disposition = TEEIO_FAULT_DISPOSITION_HOLD;
            } else {
                memcpy(m_secondary, m_held[direction], m_held_size[direction]);
            }
        }
        ...
        if (disposition == TEEIO_FAULT_DISPOSITION_DROP ||
            ...
            result.secondary_message = m_secondary;
            result.secondary_message_size = m_held_size[direction];
            m_held_size[direction] = 0;
        }
        ...
        m_previous_size[direction] = message_size;
        return result;
    }
...
const char *teeio_fault_result_record(void)
{
    snprintf(m_result, sizeof(m_result),
             "{\"scenario\":\"%s\",\"rule_id\":%d,\"disposition\":\"%s\",\"match_count\":%u,\"expected\":\"%s\",\"actual\":\"%s\",\"fired\":%s,\"fire_count\":%u}",
             m_active_scenario[0] == '\0' && m_config != NULL ?
             m_config->selected_scenario : m_active_scenario,
             m_active_rule_id,
